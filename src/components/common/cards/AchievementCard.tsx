...
import { UserBadgesDto } from "~/src/types";
...
const AchievementCard: React.FC<IAchievementProps> = ({
  headerTitle = "Badges",
  showViewAll = true,
  onViewAllPress,
  category,
        ...
        {displayImages && displayImages.length > 0 ? (
          displayImages.map((item, index) => (
            ...
            >
              ...
              <Text className="w-full text-xs text-gray-400 text-center mt-1">
                {item.stats.distanceKm} KM
              </Text>
            </View>
          ))
        ) : (
          <View className="w-full items-center py-6">
            <Text className="w-full text-center text-sm text-gray-400">No achievements yet</Text>
          </View>
        )}
      </View>
    </View>
