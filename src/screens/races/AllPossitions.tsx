...
import { RacePositionCard, StatsCard } from "~/src/components";
import { useFetch } from "~/src/hooks";
import { UserRacePositionResponse, UserRaceStatsResponse } from "~/src/types";

const AllPossitionsScreen = () => {
  ...
  const { data: raceData, isLoading: raceLoading } =
    useFetch<UserRaceStatsResponse>(
      [`my_race_stats${userId ? `_userId=${userId}` : ""}`, `${year}`],
  ...
  const { data: positionData, isLoading: positionLoading } =
    useFetch<UserRacePositionResponse>(
      [
        `my_race_positions${userId ? `_userId=${userId}` : ""}${year ? `_year=${year}` : ""}`,
      ],
      `v2/races/positions${userId ? `?userId=${userId}` : ""}${year ? `${userId ? "&" : "?"}year=${year}` : ""}`,
      {
        forceRefetch: true,
      }
    );
            ...
            >
              <View className="py-4 gap-4">
                ...
                <RacePositionCard
                  ...
                  maxImages={
                    positionLoading ? 9 : positionData?.data.data.length
                  }
                />
              </View>
            </KeyboardAwareScrollView>
...
export default AllPossitionsScreen;
