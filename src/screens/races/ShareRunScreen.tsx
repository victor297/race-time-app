...
import { formatDateToYMD, formatEventDate } from "~/src/utils";
...
const ShareRunScreen = () => {
  const { data } = useLocalSearchParams<{
    data: string;
  }>();
  const race: ShareRunType = React.useMemo(() => {
    try {
  ...
  const handleSave = async () => {
    try {
      ...
      Alert.alert("Saved", "Run image saved to your photos.");
    } catch (e) {
      console.error(e);
  ...
  const handleShare = () => {
    Alert.alert("Share Run", "Share this run to your profile?", [
      {
        text: "Cancel",
        style: "cancel",
      },
  ...
  const performShare = async () => {
    try {
      ...
      if (!capturedUri) {
        hideLoading();
        Alert.alert("Capture failed", "Couldn't capture the run image.");
        return;
      }
      const formData = new FormData();
      formData.append("location", race.location ?? "");
      formData.append("caption", race.title ?? "");
      formData.append("date", formatDateToYMD(race.date) ?? "");
      formData.append("postOnProfile", true.toString());
      formData.append("result", race.result.distance.toString());
                  ...
                  <View className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-5">
                    ...
                    <View className="flex-row justify-between items-end">
                      ...
                      <View className="flex-row gap-4">
                        ...
                        <View className="items-center">
                          <Text className="text-white/70 text-xs">Runs</Text>
                          <Text className="text-white text-xl font-bold">
                            {race.result.runs}
                          </Text>
                        </View>
